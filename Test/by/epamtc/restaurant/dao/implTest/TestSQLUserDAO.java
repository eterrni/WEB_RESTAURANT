...
public class TestSQLUserDAO implements UserDAO {

	private final ConnectionPoolTest connectionPool = ConnectionPoolTest.getInstance();

	private static final String SELECT_USER_LOGIN_PASSWORD = "SELECT * FROM `rest_db_test`.users\r\n"
			+ "WHERE users.login =? AND users.password=?;";
	private static final String ADD_NEW_USER = "INSERT INTO `rest_db_test`.`users` (`name`, `surname`, `patronymic`, `login`, `password`, `phone_number`, `age`, `email`, `users_role_id_role`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, '2');";
	private static final String UPDATE_USER_DATA = "UPDATE `rest_db_test`.`users` SET `name` = ?, `surname` = ?, `patronymic` =?, `phone_number` = ?, `age` = ?, `email` = ? WHERE (`id_users` = ?);";
